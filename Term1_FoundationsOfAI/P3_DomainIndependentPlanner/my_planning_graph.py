...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            # check if prerequisite literals hold in S[level]
            pga = PgNode_a(a)
            counter = 0;
            for pn in pga.prenodes:
                for s in self.s_levels[level]:
                    if s.__eq__(pn): counter += 1;
            if counter == len(pga.prenodes):
                # all prenodes are in the previous S[level]
                # add parents
                for pn in pga.prenodes:
                    pga.parents.add(pn)
                    for s in self.s_levels[level]:
                        if s.__eq__(pn):
                            # add children to the S[level] states
                            s.children.add(a)
                
                allactions.add(pga)
        self.a_levels.append(allactions)
    ...
    def add_literal_level(self, level):
        ...
        currslevel = set()
        # add connectivity to previous A[level-1] if appropriate
        for a in self.a_levels[level-1]:
            # put effect into the S level
            for s in a.effnodes:
                # add children to that in self.a_levels[level-1]
                a.children.add(s)
                currslevel.add(s)
        ...
        for s in currslevel:
            for a in self.a_levels[level-1]:
                if s in a.effnodes:
                    s.parents.add(a)
        self.s_levels.append(currslevel)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        return (node_a1.action.effect_add == node_a2.action.precond_pos) or (node_a1.action.precond_pos == node_a2.action.effect_add)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # if parents are mutually exclusive, then they are competing
        # thanks for forum discussion in
        # https://discussions.udacity.com/t/problem-with-test-competing-needs-mutex/227344/2
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # same symbol but one positive and the other negative
        if (node_s1.symbol == node_s1.symbol) and (node_s1.is_pos != node_s2.is_pos): return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for n in self.problem.goal:
            notfound = True; i = 0;
            while notfound:
                for s in self.s_levels[i]:
                    if n.__eq__(s.symbol) and s.is_pos == True:
                        level_sum += i;
                        notfound = 0;
                        break;
                i += 1
