...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def load_actions():
            ...
            # Load can only occur if the cargo and plane are in the same location
            # and cargo is NOT in plane
            for c in self.cargos:
                for p in self.planes:
                    for a in self.airports:
                        ...
                        curraction = Action(expr('Load({}, {}, {})'.format(c,p,a)),[precond_pos,precond_neg],[effect_add,effect_rem])
                        loads.append(curraction)
            return loads
        ...
        def unload_actions():
            ...
            # Unload can only occur if the cargo and plane are in the same location
            # and cargo is in plane
            for c in self.cargos:
                for p in self.planes:
                    for a in self.airports:
                        ...
                        curraction = Action(expr('Unload({}, {}, {})'.format(c,p,a)),[precond_pos,precond_neg],[effect_add,effect_rem])
                        unloads.append(curraction)
            return unloads
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        count = 0
        # find number of unsatisfied goals
        for g in self.goal:
            if (not (g in decode_state(node.state, self.state_map).pos)): count += 1
        return count
